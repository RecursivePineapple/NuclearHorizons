...
public class TileReactorSimulator extends TileEntity implements ITileWithModularUI {
    ...
    public Packet getDescriptionPacket() {
        ...
        if(this.latestSimulation != null) {
            nbt.setByteArray("history", this.latestSimulation.save().toByteArray());
        }
                ...
                new Row()
                    ...
                    .widgets(
                        SlotGroup.ofItemHandler(config, COL_COUNT)
                            .phantom(true)
                            .applyForWidget(slot -> {
                                slot.setChangeListener(this::onConfigChanged);
                            })
                ...
                new Row()
                    ...
                    .widgets(
                        new VanillaButtonWidget()
                            .setDisplayString(I18n.format("nh_gui.sim.actions.start"))
                            .setOnClick((d, w) -> {
                        ...
                        padding(2, 2),
                        new VanillaButtonWidget()
                            .setDisplayString(I18n.format("nh_gui.sim.actions.cancel"))
                            .setOnClick((d, w) -> {
                                if(NetworkUtils.isClient()) {
                                    simulator.cancel();
                                }
    ...
    private static void addResultLine(ArrayList<String> lines, String name, String unit, Object value) {
        var text = I18n.format(
            ...
            I18n.format(
                value == null
                    ? "nh_gui.sim.results.none"
                    : ("nh_gui.sim.results." + unit),
                value
            )
        );
                ...
                .widgets(
                    ...
                    TextWidget
                        .dynamicString(() -> {
                            ...
                            addResultLine(lines, "avg_power", "eu_per_tick", r == null ? null : (r.totalEU * 2 / 20 / r.simTime));
    ...
    private Widget booleanSetting(String name, BooleanSupplier getter, Consumer<Boolean> setter) {
        ...
        return new MultiChildWidget()
            ...
            .addChild(
                new VanillaButtonWidget()
                    .setDisplayString(getter.getAsBoolean() ? trueText : falseText)
                    .setOnClick((t, u) -> {
                    ...
                    .setTicker(w -> ((VanillaButtonWidget)w).setDisplayString(getter.getAsBoolean() ? trueText : falseText))
                    .setSize(40, 16)
                    .setPos(98, 0)
            );
    }
    ...
    private Widget reactorSettingsPage() {
        return new Row()
            .widgets(
                padding(7, 7),
                new Column()
                    .setAlignment(MainAxisAlignment.START, CrossAxisAlignment.START)
                    .widgets(
                        padding(7, 7),
                        new Row()
                            .setAlignment(MainAxisAlignment.CENTER)
                            .widget(
                                new TextWidget(I18n.format("nh_gui.sim.settings.title")).setSize(150, 16)
                            ),
                        padding(7, 7),
                        textSetting(
                        ...
                        padding(4, 4),
                        booleanSetting(
                            I18n.format("nh_gui.sim.settings.fluid"),
                            () -> this.config.fluid,
                            b -> {
                                this.config.fluid = b;
                                onConfigChanged();
                        ...
                        padding(4, 4),
                        booleanSetting(
                            I18n.format("nh_gui.sim.settings.pulsed"),
                            () -> this.config.pulsed,
                            b -> {
                        ...
                        padding(4, 4),
                        numericSetting(
                            I18n.format("nh_gui.sim.settings.on_pulse"),
                            () -> this.config.onPulse,
                            v -> {
                        ...
                        padding(4, 6),
                        numericSetting(
                            I18n.format("nh_gui.sim.settings.off_pulse"),
                        ...
                        padding(4, 6),
                        numericSetting(
                            I18n.format("nh_gui.sim.settings.resume_temp"),
                ...
                new Column()
                    ...
                    .widgets(
                        ...
                        new Row()
                            .setAlignment(MainAxisAlignment.CENTER)
                            .widget(
                                new TextWidget(I18n.format("nh_gui.sim.comp_settings.title")).setSize(150, 16)
                            ),
                        padding(7, 7),
                        TextWidget
                            .dynamicString(() -> {
                                ...
                                if(activeComponentX == -1 || activeComponentY == -1) {
                                    ...
                                    active = I18n.format(
                                        ...
                                        activeComponentY,
                                        comp == null
                                            ? I18n.format("nh_gui.sim.results.none")
    ...
    private static class SimulatorSlotWidget extends SlotWidget {
        ...
        public java.util.List<String> getExtraTooltip() {
            ...
            tooltip.add(EnumChatFormatting.DARK_GRAY.toString() + I18n.format("nh_gui.sim.actions.edit_comp_automation"));
    ...
    private static class MessageChannel<FromServer extends IMessage, FromClient extends IMessage> extends SyncedWidget {
        ...
        public MessageChannel(
            @Nullable Class<? extends FromServer> from_server,
            @Nullable Consumer<FromServer> handle_on_client,
            @Nullable Class<? extends FromClient> from_client,
        ...
        public void readOnClient(int arg0, PacketBuffer arg1) throws IOException {
            if(from_server != null && handle_on_client != null) {
                ...
                try {
                    ...
                    throw new RuntimeException("Could not create instance of FromServer message; type must have a public no-argument constructor", e);
                }
        ...
        public void readOnServer(int arg0, PacketBuffer arg1) throws IOException {
            if(from_client != null && handle_on_server != null) {
                ...
                try {
                    ...
                    throw new RuntimeException("Could not create instance of FromClient message; type must have a public no-argument constructor", e);
                }
